...
function NearbyUserMarker({
  ...
  const label = `${person.name}, ${person.distanceMeters} meters away${
...
function NearbyEmptyState() {
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center">
      ...
      <p className="mt-3 text-sm font-semibold">No one nearby</p>
      <p className="mt-1 text-xs text-muted-foreground">People within 100m will appear here.</p>
    </div>
  );
}
...
function LocationUnavailableState() {
  return (
    <div className="flex h-[320px] flex-col items-center justify-center rounded-3xl border border-dashed border-border bg-surface/60 px-6 text-center">
      ...
      <p className="mt-3 text-sm font-semibold">See who's nearby</p>
      <p className="mt-1 text-xs text-muted-foreground">
        Turn on location to see people around you.
      </p>
    </div>
  );
}
        ...
        {person && (
          <>
            <SheetHeader className="px-0 text-left">
              <SheetTitle className="sr-only">Nearby preview for {person.name}</SheetTitle>
            </SheetHeader>
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-muted-foreground">
                  {person.isFriend ? "Friend" : "Nearby"} · {person.distanceMeters}m away
                </p>
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
              ...
              <div className="rounded-2xl border border-border/70 bg-surface-2 p-3">
                ...
                <p className="mt-1 line-clamp-2 text-muted-foreground">
                  {person.interests.slice(0, 2).join(", ")} · {person.match}% match
                </p>
              </div>
            </div>
            ...
            <p className="mt-3 flex items-center justify-center gap-1 text-center text-[10px] text-muted-foreground">
              ...
              Only rounded distance is shown. No coordinates or exact address.
            </p>
          </>
        )}
      </SheetContent>
    ...
    <section className="mt-5">
      <div className="mb-3 flex items-end justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Within 100m</p>
        </div>
      ...
      <p className="mt-2 flex items-center justify-center gap-1.5 text-center text-[11px] text-muted-foreground">
        ...
        Relative social proximity only. No maps, routes, coordinates, or precise addresses.
      </p>
